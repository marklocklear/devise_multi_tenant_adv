class TasksController < ApplicationController
  def index
    @tasks = Task.get_tasks(current_user) #multi-tenancy; only returns tasks with the the current users org
  end

#if conditional below only allows user to view model if it is returned via the get_tasks method
 def show
    if @task = Task. get_tasks(current_user).select {|p| p.id.eql?(params[:id].to_i) }.first
